...
file_handler.setFormatter(
    logging.Formatter("%(asctime)s | %(levelname)s | IP: %(message)s")
)
...
def ratelimit_handler(e):
    ...
    log_security_event(
        "RATE_LIMIT_EXCEEDED",
        f"Threshold reached: {e.description}",
        get_remote_address(),
    )
...
@limiter.limit(
    "5 per minute"
)  # Throttles authentication attempts to 5 per minute per IP
def login():
    if request.method == "POST":
        username = request.form["username"].strip()
        password = request.form["password"]
        ...
        if user:
            ...
            if time.time() < user["locked_until"]:
                ...
                log_security_event(
                    "ACCOUNT_LOCKOUT",
                    f"Blocked access to locked account: {username}",
                    get_remote_address(),
                )
            ...
            if bcrypt.checkpw(password.encode(), user["password"].encode()):
                ...
                if new_attempts >= MAX_ATTEMPTS:
                    ...
                    flash(
                        f"Too many failed attempts. Account locked for {LOCK_TIME} seconds.",
                        "danger",
                    )
                else:
...
def change_password():
    if request.method == "POST":
        ...
        if new_password == current_password:
            conn.close()
            flash(
                "New password must be different from your current password.", "danger"
            )
            return render_template("change_password.html")
        ...
        conn.execute(
            "UPDATE users SET password = ? WHERE id = ?", (hashed, session["user_id"])
        )
        conn.commit()
        conn.close()
...
def view_logs():
    ...
    try:
        if os.path.exists(log_file_path):
            ...
            log_list = [
                "Audit log file not found. System has not recorded any events yet."
            ]
...
def admin():
    ...
    return render_template(
        "admin.html", students=student_list, grades=grade_list, teachers=teachers
    )
...
def assign_grade():
    ...
    redirect_target = (
        url_for("admin") if session.get("role") == "admin" else url_for("teacher")
    )
...
def edit_grade(grade_id):
    ...
    conn = get_db()
    # Verify grade exists and teacher has permission to edit it
    if session.get("role") == "teacher":
        existing = conn.execute(
            "SELECT id FROM grades WHERE id = ? AND assigned_by = ?",
...
def delete_grade(grade_id):
    ...
    if session.get("role") == "teacher":
        result = conn.execute(
            "DELETE FROM grades WHERE id = ? AND assigned_by = ?",
            (grade_id, session["user_id"]),
        )
    else:
        result = conn.execute("DELETE FROM grades WHERE id = ?", (grade_id,))
...
def delete_user(user_id):
    ...
    if user and user["role"] == "admin":
        ...
        log_security_event(
            "ADMIN_ACTION",
            f"Admin '{session['username']}' deleted user: {user['username']}",
            get_remote_address(),
        )
...
def create_teacher():
    ...
    try:
        ...
        log_security_event(
            "ADMIN_ACTION",
            f"Admin '{session['username']}' created teacher account: {username}",
            get_remote_address(),
        )
        conn.close()
...
def delete_teacher(user_id):
    ...
    if user and user["role"] == "teacher":
        ...
        log_security_event(
            "ADMIN_ACTION",
            f"Admin '{session['username']}' deleted teacher: {teacher_name}",
            get_remote_address(),
        )
        conn.commit()
...
def unlock_user(user_id):
    ...
    log_security_event(
        "ADMIN_ACTION",
        f"Admin '{session['username']}' unlocked user ID: {user_id}",
        get_remote_address(),
    )
    conn.close()
